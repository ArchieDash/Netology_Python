...
def find_friends(token, user_id):
...
def groups_info(token, friends, groups):
    progress = 1
...
def main():
    ...
    user = user_name()
    result(target_groups(groups_info(token, find_friends(token, user), find_groups(token, user))))
